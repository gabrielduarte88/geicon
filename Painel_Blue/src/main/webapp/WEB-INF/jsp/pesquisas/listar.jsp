        ...
        <div class="acoes">
            <div class="botoes">
                <c:if test="${usuarioAtual.administrador}">
                    <button class="btn" id="btnNovo"><i class="fa fa-plus-circle"></i> nova pesquisa</button>
                </c:if>
            </div>
            <div class="filtro">
                <span>Buscar em</span>
                <select id="campoFiltro">
                    ...
                    <option value="responsavel.nome" ref="string" ${campoFiltro == 'responsavel.nome' ? 'selected' : ''}>responsável</option>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th>
                        Responsável
                        <a href="#" class="ordem" ref="responsavel.nome" title="Ordenar" data-toggle="tooltip">
                            <tmpl:sort-icon field="responsavel.nome" sortField="${campoOrdem}" order="${ordem}" />
                        </a>
                    </th>
                    <th>
                        Data
            ...
            <tbody>
                <c:forEach var="pesquisa" items="${pesquisas}">
                    <tr>
                        <th scope="row">
                            <a href="#" class="visualizar" ref="${pesquisa.code}"><fmt:formatNumber pattern="000000" value="${pesquisa.id}" /></a>
                        </th>
                        <td>${pesquisa.nome}</td>
                        <td>${pesquisa.responsavel.nome}</td>
                        <td class="text-center"><fmt:formatDate type="both" timeStyle="short" value="${pesquisa.data}" /></td>
                        <td class="text-center">
                            <a href="#" class="visualizar" ref="${pesquisa.code}">
                                <button class="btn btn-primary"><i class="fa fa-edit"></i> editar</button>
                            </a>
                        </td>
                    </tr>
                </c:forEach>
                <c:if test="${empty pesquisas}">
                    <tr>
            ...
            <tfoot>
                <tr>
                    <td colspan="6">
                        ...
                        <c:if test="${total > 0}">
                            Exibindo ${fn:length(pesquisas)} de ${total} itens.
                        </c:if>
                    </td>
                </tr>
            </tfoot>
